...
```{python}
# Print list of countries in WDPA dataset 
print(set(wdpa["ISO3_Int"]))
```

```{python}
# Save this list of parks for reference later to show the number of parks excluded. 
parks_dropped = wdpa[wdpa['GIS_AREA'] <= .3]
...
```{python}
# CHECK: Count and % of parks dropped 
print("# Parks Dropped: ", len(parks_dropped))
print("# Parks for Cost Estimates: ", len(wdpa) - len(parks_dropped))
print("% Parks Dropped: ", len(parks_dropped)/len(wdpa))
    # NOTE: Take-away for external validity is that we CANNOT discuss neighborhood parks, but rather are making assessments about large natural parks
print("KM^2 of parks not included: ", np.sum(parks_dropped['GIS_AREA']))
print("% of park area droppped: ", np.sum(parks_dropped['GIS_AREA'])/np.sum(wdpa['GIS_AREA']))
    # NOTE: This is key to the justification for dropping below resolution. While 47% of parks are dropped below resolution, they encompass only 0.015% of the total park areas. So even aggreagated they are insignificant.
```
